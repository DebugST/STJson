...
namespace STLib.Json
{
    ...
    {
        public static Dictionary<string, STJsonPathCustomFuncHander> CustomFunctions { get; private set; }
        ...
        static STJsonPath() {
            ...
            ME.CustomFunctions = new Dictionary<string, STJsonPathCustomFuncHander>();
        ...
        private static STJson GetParseExpressFunction(ExpressionToken token) {
        ...
        internal static void GetSTJsons(List<STJsonPathItem> items, int nIndex, STJson jsonCurrent, SelectSetting setting, STJson jsonOut) {
            ...
            int nLoopIndex = 0;
            ExpressionToken exp_ret = ExpressionToken.Undefined;
            ...
            switch (item.Type) {
                ...
                case STJsonPathItem.ItemType.Express:
                    ...
                    if (exp_ret.Type == ExpressTokenType.Undefined || exp_ret.Value == null) {
                        return;
                    }
                    if (item.Express.IsFilter) {
                        ...
                        if (jsonCurrent == null) return;
                        if (exp_ret.Type == ExpressTokenType.Long) {
                            if (jsonCurrent.ValueType != STJsonValueType.Array) {
                                return;
                            }
                        ...
                        if (exp_ret.Type == ExpressTokenType.String) {
                        ...
                        if (exp_ret.Type == ExpressTokenType.Array) {
                            foreach (var v in exp_ret.Value as ExpressionToken[]) {
                                switch (jsonCurrent.ValueType) {
                                    case STJsonValueType.Object:
                                        if (v.Type != ExpressTokenType.String || v.Value == null) {
                                            continue;
                                        }
                                    ...
                                    case STJsonValueType.Array:
                                        if (v.Type != ExpressTokenType.Long || v.Value == null) {
                                            continue;
                                        }
