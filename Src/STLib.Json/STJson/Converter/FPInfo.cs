...
namespace STLib.Json
{
    internal class FPInfo
    {
        ...
        public static List<FPInfo> GetFPInfo(Type type) {
            ...
            if (m_dic_fp_infos.ContainsKey(n_code)) {
                return m_dic_fp_infos[n_code];
            } else {
                lst = new List<FPInfo>();
                m_dic_fp_infos.Add(n_code, lst);
            }
            ...
            for (int i = 0; i < lst.Count; i++) {
                ...
                n_code = attr.GetType().GetHashCode();
                if (m_dic_type_map.ContainsKey(n_code)) {
                    converter = m_dic_type_map[n_code];
                } else {
                    converter = (STJsonConverter)Activator.CreateInstance(((STJsonConverterAttribute)attr).Type);
                    m_dic_type_map.Add(n_code, converter);
                }
